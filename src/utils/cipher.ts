...
const { VITE_JWT_APPID } = getAppEnvConfig();
...
export function encryptByHmacSHA256(password: string, key: string) {
  const BinaryArray = HmacSHA256(password, key);
  return Base64.stringify(BinaryArray).toLocaleUpperCase();
}
export function encryptByBase64(cipherText: string) {
  return UTF8.parse(cipherText).toString(Base64);
}
export function encryptPswByBase64(password: string, username?: string): string {
  let result: string = '';
  if (!username) {
    result = encryptByBase64(password);
  }
  if (username && password) {
    const baseUserPass = encryptByBase64(password + username);
    result = encryptByBase64(VITE_JWT_APPID + baseUserPass);
  }
  return result;
}
